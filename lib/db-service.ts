...
export const dbService = {
  ...
  async fetchCostSectors(): Promise<CostSector[] | null> {
    ...
    if (!supabase || !isSupabaseConfigured()) {
      return [
        { id: 'sec-001', code: 'SET-PROD', name: 'Fabricação & Mistura Artesanal', costCenterId: 'proc-cc-001', active: true },
        { id: 'sec-002', code: 'SET-ENV', name: 'Envase & Linha de Embalagem', costCenterId: 'proc-cc-002', active: true },
        { id: 'sec-003', code: 'SET-MANUT', name: 'Manutenção & Engenharia de Fábrica', costCenterId: 'proc-cc-003', active: true },
        { id: 'sec-004', code: 'SET-ALMOX', name: 'Pesagem & Almoxarifado', costCenterId: 'proc-cc-004', active: true },
      ];
    }

    const { data, error } = await supabase.from('cost_sectors').select('*').order('name');
    if (error || !data || data.length === 0) {
      // Fallback padrão se a tabela estiver vazia
      const defaultSectors: CostSector[] = [
        { id: 'sec-001', code: 'SET-PROD', name: 'Fabricação & Mistura Artesanal', costCenterId: 'proc-cc-001', active: true },
        { id: 'sec-002', code: 'SET-ENV', name: 'Envase & Linha de Embalagem', costCenterId: 'proc-cc-002', active: true },
        { id: 'sec-003', code: 'SET-MANUT', name: 'Manutenção & Engenharia de Fábrica', costCenterId: 'proc-cc-003', active: true },
        { id: 'sec-004', code: 'SET-ALMOX', name: 'Pesagem & Almoxarifado', costCenterId: 'proc-cc-004', active: true },
  ...
  async saveCostSector(sector: CostSector): Promise<boolean> {
    if (typeof window !== 'undefined') {
      try {
        ...
        console.warn('Erro ao salvar setor no localStorage:', e);
      }
    }
